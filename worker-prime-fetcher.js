...
function getTargetEndpoint(env) {
  return cleanText(env?.TARGET_ENDPOINT);
}
...
async function triggerValTownPipeline(env) {
  try {
    ...
    if (!endpoint) {
      console.log("TARGET_ENDPOINT missing; skipping Val Town trigger.");
      return {
        ok: false,
        skipped: true,
        reason: "Missing TARGET_ENDPOINT",
      };
    }
    ...
    if (!res.ok) {
      console.log("Val Town trigger failed:", res.status, text);
      return {
        ok: false,
        skipped: true,
    ...
    console.log("Val Town trigger error:", err);
    return {
      ok: false,
      skipped: true,
...
function getEnglishTitle(media) {
  return (
    ...
    cleanText(
      media?.titles?.find(
        (t) => cleanText(t?.type)?.toLowerCase() === "english"
      )?.title
    ) ||
